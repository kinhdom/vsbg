...
class NewPost extends Component {
    ...
    updateDatabase() {
        ...
        console.log(params)
        axios.post('http://localhost:3000/api/updataDatabase', params)
            .then(res => {
                console.log(res)
            })
    ...
    crawlDatabase() {
        ...
        axios.post('http://localhost:3000/api/crawl', querystring.stringify(dataCrawl))
    }
    fetchData(group_id, page) {
        ...
        return axios.post('http://localhost:3000/api/newpost/', querystring.stringify(dataFetch))
            .then(res => {
                    ...
                    {this.state.arrPosts.map((post) => {
                        return (
                            <Post
                                ...
                                score={post.score}
                                user_fb_access_token={this.state.user_fb_access_token}
                            />
                        )
                    })}
                </div>
    ...
    componentWillUpdate(nextProps) {
        ...
        let group_info = nextProps.match.params
    ...
    componentDidMount() {
        ...
        let group_id = this.props.match.params.group_id
        axios.post('http://localhost:3000/api/userinfo', querystring.stringify({ user_id: user_id }))
            .then(info => {
